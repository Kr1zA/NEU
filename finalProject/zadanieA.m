function [retVal] = zadanieA(normalizovaneBurkyNaUcenie, normalizovaneDataNaTestovanie)
...
w3 = rand(10, 1) * 2 - 1; 
  ...
  for k = 0:pocetBurok - 1
    ...
    for m = 0:100
      ...
      for j = 1:144 - 10
        ...
        X = [-1; storm(j:j + 7, 1); storm(j:j + 7, 2); storm(j:j + 7, 3); storm(j:j + 7, 4)]; 
  ...
  for j = 1:size(dataNaTestovanie) - 10
        X = [-1; dataNaTestovanie(j:j + 7, 1); dataNaTestovanie(j:j + 7, 2); dataNaTestovanie(j:j + 7, 3); dataNaTestovanie(j:j + 7, 4)]; 
        #pocitanie vystupov 1 vnutornej vrstvy
        V1 = X' * w1;  
        V11 = [-1,V1];
        V1 = tanh(V1); 
